...
class MyAgent(ACTR):
    ...
    DMbuffer=Buffer()                   
    DM=Memory(DMbuffer,latency=0.05,threshold=2)     # latency controls the relationship between activation and recall
                                                     # activation must be above threshold - can be set to none
            
    dm_n=DMNoise(DM,noise=0.0,baseNoise=0.5)         # turn on for DM subsymbolic processing
